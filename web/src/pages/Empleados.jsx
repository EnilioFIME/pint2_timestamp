              ...
              filteredEmpleados.map((empleado) => (
                <>
                  <tr key={empleado.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 font-mono text-sm font-medium text-gray-900">{empleado.id}</td>
                    <td className="px-6 py-4 font-medium text-gray-900">{empleado.nombre}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${getEstadoColor(empleado.estado)}`}>
                        {empleado.estado}
                      </span>
                    </td>
                    <td className="px-6 py-4">{renderBadge(empleado.nfc)}</td>
                    <td className="px-6 py-4">{renderBadge(empleado.biometrico)}</td>
                    <td className="px-6 py-4">
                      <div className="flex gap-3">
                        <button
                          onClick={() => { setEditingEmpleado(empleado); setShowModal(true); }}
                        ...
                        <button
                          onClick={() => setConfirmDeleteId(empleado.id)}
                  ...
                  {confirmDeleteId === empleado.id && (
                    <tr key={`confirm-${empleado.id}`} className="bg-red-50">
                      <td colSpan="6" className="px-6 py-3">
                        <div className="flex items-center gap-3">
                          <AlertTriangle size={16} className="text-red-500 flex-shrink-0" />
                          <span className="text-sm text-red-700">
                            ¿Eliminar a <strong>{empleado.nombre}</strong>? Esta acción no se puede deshacer.
                          </span>
      ...
      <div className="bg-white rounded-lg w-full max-w-md">
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-4 space-y-5">
            ...
            <div
              onClick={() => setNfcActivado(!nfcActivado)}
            ...
            <div
              onClick={() => setBiometricoActivado(!biometricoActivado)}
              className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-colors ${
