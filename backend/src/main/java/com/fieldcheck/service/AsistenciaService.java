...
public class AsistenciaService {
    ...
    public PageResponse<AsistenciaDTO> findWithFilters(LocalDateTime desde, LocalDateTime hasta,
                                                       Long idProyecto, Long idUsuario,
                                                       Pageable pageable) {
        Page<Asistencia> pageResult = asistenciaRepository.findWithFilters(desde, hasta, idProyecto, idUsuario, pageable);
        List<Asistencia> rows = pageResult.getContent();

        // Batch-load para evitar N+1 solo con los registros de esta página
        ...
        List<AsistenciaDTO> dtos = rows.stream().map(a -> {
            ...
            return new AsistenciaDTO(
                    ...
                    usuarioResumen, proyectoResumen
            );
        }).toList();
